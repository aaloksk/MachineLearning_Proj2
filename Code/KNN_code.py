# -*- coding: utf-8 -*-
"""
Created on Sun Mar  5 15:17:01 2023
...
path = 'C:\\Users\\Aalok\\OneDrive - lamar.edu\\0000CVEN6301_ML\\Project2\\Dockum_ET'
os.chdir(path)
...
#Importiung scv with fishnet id and merging fishnet id to preds table
FNid = pd.read_csv('Wells_FN.csv')
FNid['WellID'] = FNid.StateWellNumber
a = pd.merge(a, FNid[['WellID','FN_ID']], on='WellID', how='left')
a.head()


# randomly sample one row from each group and store in a new DataFrame
a_train = a.groupby('FN_ID').apply(lambda x: x.sample(1, random_state=42)).reset_index(drop=True)

# store the remaining rows in a separate DataFrame
a_test = a[~a.index.isin(a_train.index)]
...
#X = a[a.columns[cols]]
#X
X_train = a_train[a_train.columns[cols]]
X_test = a_test[a_test.columns[cols]]
...
#Y = b['predLR']
#Y2 = b['pred_NB']
Y_train = a_train['predLR']
Y_test = a_test['predLR']

Y_train2 = a_train['pred_NB']
Y_test2 = a_test['pred_NB']
...
#Mesh of all possible points across the aquifer
ally, allx = np.meshgrid(lon_all, lat_all)
all_points = pd.DataFrame(dict(x=allx.ravel(), y=ally.ravel()))
all_points = all_points.rename(columns={'x':'LatDD', 'y':'LonDD'})
plt.plot(all_points.LatDD, all_points.LonDD)
